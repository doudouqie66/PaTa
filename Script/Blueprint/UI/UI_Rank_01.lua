---@class UI_Rank_01_C:UUserWidget
...
--Edit Below--
local UI_Rank_01 = { bInitDoOnce = false }

--[[----------------------刷新最短爬塔时间排行榜------------------------]]
function UI_Rank_01:RefreshRankingList(Rank_List_Data, Ranking_List_Manager, Rank_ID)
    ...
    local Time_Texts = {self.Time_1, self.Time_2, self.Time_3, self.Time_4, self.Time_5, self.Time_6, self.Time_7, self.Time_8, self.Time_9, self.Time_10} -- 用时文本
    ...
    for Index = 1, 10 do
        ...
        if Rank_Data then
            ...
            local Score = math.max(0, math.floor(Rank_Data.Score or 0)) -- 爬塔用时秒数
            Rank_Texts[Index]:SetText(tostring(Rank_Data.Rank or Index))
            Player_Texts[Index]:SetText(Player_Name)
            Time_Texts[Index]:SetText(string.format("%02d:%02d", Score // 60, Score % 60))
        else
            Rank_Texts[Index]:SetText(tostring(Index))
            Player_Texts[Index]:SetText("--")
            Time_Texts[Index]:SetText("--:--")
        end
    end
end

-- function UI_Rank_01:Tick(MyGeometry, InDeltaTime)

-- end

-- function UI_Rank_01:Destruct()
